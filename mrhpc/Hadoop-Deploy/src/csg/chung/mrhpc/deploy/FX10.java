...
public class FX10 {
	...
	public void startSlaves(int rank){
		try {
			...
			if (rank <= Configure.NUMBER_OF_DATANODE){
				...
				runCommand(FX10.HADOOP_FOLDER + rank + "/sbin/yarn-daemon.sh start nodemanager");
			}
	...
	public void generateCode(int rank, String masterAddress){
		try {
			...
			runCommand("tar -zxf " + HADOOP_TAR_GZ_FILE + " --directory=" + FX10.HADOOP_FOLDER + " --transform s/hadoop/" + rank + "/");
